...
async def do_math(svg_elem, cmd):
    ...
    div.style.fontSize = cmd.get("font-size", "16px")
...
async def apply_svg_command(svg_elem, cmd):
    ...
    if ctype == "text" and "text" in cmd:
        ...
        # If there are any line breaks, split into lines
        if "y" not in cmd:
            _ui_current_y=_ui_current_y - (_ui_line_size * len(raw_txt.split(chr(13))))
...
class ExplainerPlayer:
    def __init__(self, command_list):
        ...
        if not document.getElementById("mysvg"):
            doc_svg=svg.svg(id="mysvg", width=SVG_WIDTH, height=SVG_HEIGHT)
